    ...
    public ItemStack apply(ItemStack stack, Random rand, LootContext context) {
        if (!stack.isEmpty() && context.getLootedEntity() instanceof EntityDragonBase) {
            ...
            if (stack.getItem() instanceof ItemDragonFlesh) {
                stack.setCount(1 + random.nextInt(1 + (dragon.getAgeInDays() / 25)));
                return new ItemStack(dragon.dragonType == DragonType.FIRE ? ModItems.fire_dragon_flesh : ModItems.ice_dragon_flesh, stack.getCount(), stack.getMetadata());
            }
            if (stack.getItem() instanceof ItemDragonSkull) {
                ItemStack stack1 = new ItemStack(dragon.dragonType == DragonType.FIRE ? ModItems.dragon_skull : ModItems.dragon_skull, stack.getCount(), stack.getMetadata());
            ...
            if (stack.getItem() == ModItems.fire_dragon_blood || stack.getItem() == ModItems.ice_dragon_blood) {
                return new ItemStack(dragon.dragonType == DragonType.FIRE ? ModItems.fire_dragon_blood : ModItems.ice_dragon_blood, stack.getCount(), stack.getMetadata());
            }
            if (stack.getItem() == ModItems.fire_dragon_heart || stack.getItem() == ModItems.ice_dragon_heart) {
                return new ItemStack(dragon.dragonType == DragonType.FIRE ? ModItems.fire_dragon_heart : ModItems.ice_dragon_heart, stack.getCount(), stack.getMetadata());
            }
        }
        return stack;
    }
