...
public class CraftTweakerCompat {
    ...
    public static void addFireDragonForgeRecipe(IItemStack iinput, IItemStack ibloodinput, IItemStack ioutput) {
        ModRecipes.FIRE_FORGE_RECIPES.add(new DragonForgeRecipe(CraftTweakerMC.getItemStack(iinput), CraftTweakerMC.getItemStack(ibloodinput), CraftTweakerMC.getItemStack(ioutput)));
    }

    @ZenMethod
    public static void addIceDragonForgeRecipe(IItemStack iinput, IItemStack ibloodinput, IItemStack ioutput) {
        ModRecipes.ICE_FORGE_RECIPES.add(new DragonForgeRecipe(CraftTweakerMC.getItemStack(iinput), CraftTweakerMC.getItemStack(ibloodinput), CraftTweakerMC.getItemStack(ioutput)));
    }
    ...
    public static void removeFireDragonForgeRecipe(IItemStack ioutput) {
        ...
        output.setCount(1);
        ModRecipes.FIRE_FORGE_RECIPES.removeIf(recipe -> recipe.getOutput().copy().isItemEqual(output));
    }
    ...
    public static void removeIceDragonForgeRecipe(IItemStack ioutput) {
        ItemStack output = CraftTweakerMC.getItemStack(ioutput).copy();
        output.setCount(1);
        ModRecipes.ICE_FORGE_RECIPES.removeIf(recipe -> recipe.getOutput().copy().isItemEqual(output));
    }
