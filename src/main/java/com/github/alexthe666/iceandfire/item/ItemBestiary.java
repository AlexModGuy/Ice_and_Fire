...
public class ItemBestiary extends Item{
	...
	public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean f) {
		if(stack.getTagCompound() != null){
			...
			while(itr.hasNext()) {
				list.add(EnumChatFormatting.WHITE + "-" + StatCollector.translateToLocal("bestiary." + EnumBestiaryPages.values()[((EnumBestiaryPages)itr.next()).ordinal()].toString().toLowerCase()));
			}
