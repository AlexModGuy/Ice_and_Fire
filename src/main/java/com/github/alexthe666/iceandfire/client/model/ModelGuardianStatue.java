...
public class ModelGuardianStatue extends SegmentedModel {
    ...
    public void setupAnim(Entity entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        for (int i = 0; i < 12; ++i) {
            ...
            this.guardianSpines[i].x = afloat3[i] * (1.0F + MathHelper.cos(ageInTicks * 1.5F + (float) i) * 0.01F - f1);
            this.guardianSpines[i].y = 16.0F + afloat4[i] * (1.0F + MathHelper.cos(ageInTicks * 1.5F + (float) i) * 0.01F - f1);
            this.guardianSpines[i].z = afloat5[i] * (1.0F + MathHelper.cos(ageInTicks * 1.5F + (float) i) * 0.01F - f1);
        }
        ...
        this.guardianTail[0].yRot = MathHelper.sin(f2) * (float) Math.PI * 0.05F;
        this.guardianTail[1].yRot = MathHelper.sin(f2) * (float) Math.PI * 0.1F;
        this.guardianTail[1].x = -1.5F;
        this.guardianTail[1].y = 0.5F;
        this.guardianTail[1].z = 14.0F;
        this.guardianTail[2].yRot = MathHelper.sin(f2) * (float) Math.PI * 0.15F;
