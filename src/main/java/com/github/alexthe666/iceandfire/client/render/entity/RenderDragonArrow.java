...
public class RenderDragonArrow extends Render {
	...
	public void doRender(EntityDragonArrow entity, double x, double y, double z, float yaw, float partialTicks) {
		...
		buffer.pos(-7.0D, -2.0D, -2.0D).tex(f4, f6).endVertex();
		buffer.pos(-7.0D, -2.0D, 2.0D).tex(f5, f6).endVertex();
		buffer.pos(-7.0D, 2.0D, 2.0D).tex(f5, f7).endVertex();
		buffer.pos(-7.0D, 2.0D, -2.0D).tex(f4, f7).endVertex();
		tessellator.draw();
		GL11.glNormal3f(-f8, 0.0F, 0.0F);
		buffer.begin(7, DefaultVertexFormats.POSITION_TEX);
		buffer.pos(-7.0D, 2.0D, -2.0D).tex(f4, f6).endVertex();
		...
		for (int j = 0; j < 4; ++j) {
			...
			buffer.begin(7, DefaultVertexFormats.POSITION_TEX);
			buffer.pos(-8.0D, -2.0D, 0.0D).tex(f, f2).endVertex();
			buffer.pos(8.0D, -2.0D, 0.0D).tex(f1, f2).endVertex();
			buffer.pos(8.0D, 2.0D, 0.0D).tex(f1, f3).endVertex();
			buffer.pos(-8.0D, 2.0D, 0.0D).tex(f, f3).endVertex();
			tessellator.draw();
		}
