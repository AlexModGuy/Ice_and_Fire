    ...
    public static class Handler implements IMessageHandler<MessageModKeys, IMessage> {
        ...
        public IMessage onMessage(MessageModKeys message, MessageContext ctx) {
            if (ctx.side == Side.SERVER) {
				 ...
				 * (EntityDragonBase)
				 * ctx.getServerHandler().playerEntity.getRidingEntity(); if
				 * (message.keyId == 1) { System.out.println("Up"); //
				 * dragon.motionY += 0.1D; } if (message.keyId == 2) {
				 * System.out.println("Down"); } if (message.keyId == 3) { if
				 * (!dragon.isBreathingFire) dragon.isBreathingFire = true;
				 * dragon.ticksTillStopFire = 20; if (dragon.getAnimation() ==
				 * IAnimatedEntity.NO_ANIMATION) {
				 * dragon.setAnimation(EntityDragonBase.animation_flame); }
				 * 
				 * } if (message.keyId == 4) { if (dragon.attackTick == 0) {
				 * dragon.attackTick = 1; } if (dragon.getAnimation() ==
				 * IAnimatedEntity.NO_ANIMATION) {
