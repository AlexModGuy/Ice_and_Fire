...
public class FireChargeExplosion extends Explosion {
    ...
    public void doExplosionB(boolean spawnParticles) {
        ...
        if (this.isSmoking) {
            for (BlockPos blockpos : this.affectedBlockPositions) {
                ...
                if (spawnParticles) {
                    ...
                    d4 = d4 * d7;
                    d5 = d5 * d7;
                    IceAndFire.PROXY.spawnParticle("snowflake", worldObj, (d0 + this.explosionX) / 2.0D, (d1 + this.explosionY) / 2.0D, (d2 + this.explosionZ) / 2.0D, d3, d4, d5);
        ...
        if (this.isFlaming) {
            ...
            for (BlockPos blockpos1 : this.affectedBlockPositions) {
                if (this.worldObj.getBlockState(blockpos1).getMaterial() == Material.AIR && this.worldObj.getBlockState(blockpos1.down()).isFullBlock() && this.explosionRNG.nextInt(3) == 0) {
                    this.worldObj.setBlockState(blockpos1, Blocks.SNOW_LAYER.getDefaultState());
                }
            }
        }
    }
