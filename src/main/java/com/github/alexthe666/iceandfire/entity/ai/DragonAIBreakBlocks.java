	...
	public void updateTask() {
		if (shouldExecute()) {
			for (int a = (int) Math.round(dragon.getEntityBoundingBox().minX) - 1; a <= (int) Math.round(dragon.getEntityBoundingBox().maxX) + 1; a++) {
				for (int b = (int) Math.round(dragon.getEntityBoundingBox().minY) + (dragon.isFlying() || dragon.isHovering() ? 0 : 1); (b <= (int) Math.round(dragon.getEntityBoundingBox().maxY) + 2) && (b <= 127); b++) {
