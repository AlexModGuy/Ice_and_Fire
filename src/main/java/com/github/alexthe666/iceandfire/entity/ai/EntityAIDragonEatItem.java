...
public class EntityAIDragonEatItem extends EntityAIBase {
	...
	public boolean shouldExecute() {
		...
		if (fooditem != null) {
			...
			path = this.dragon.getNavigator().getPathToXYZ(this.destX, this.destY, this.destZ); // this.dragon.getNavigator().tryMoveToXYZ(this.destX,
																								// this.destY,
																								// this.destZ,
																								// 1.0D);
			return path != null;
	...
	public void updateTask() {
		...
		/*
		 * if (this.fooditem != null && this.fooditem.isEntityAlive()) { int i =
		 * this.dragon.pickUpFood(this.fooditem.getEntityItem());
		 * 
		 * if (i > 0) { this.fooditem.getEntityItem().stackSize = i; endTask();
		 * } else { this.fooditem.setDead(); endTask(); } }
		 */
